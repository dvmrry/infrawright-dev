"""Tests for engine.gen_module renderers, pinned to the committed dumps."""
import json
import os
import shutil
import tempfile
...
class RenderRestTest(unittest.TestCase):
    ...
    def test_every_generated_versions_pins_its_provider(self):
        ...
        for resource_type in generated_types():
            ...
            self.assertIn(
                "      version = ", out,
                "%s/versions.tf has no provider version pin — extend the "
                "pack schema-extract provider pin source" % resource_type,
            )
...
class GoldenTest(unittest.TestCase):
    ...
    def test_rendered_output_matches_blessed_goldens(self):
        ...
        import engine.gen_module as g
        for resource_type, fname, renderer in self.GOLDENS:
            path = os.path.join("tests", "fixtures", "gen", resource_type, fname)
...
class AllComputedBlockTest(unittest.TestCase):
    ...
    def test_render_main_skips_all_computed_block(self):
        from engine.gen_module import render_main
    ...
    def test_render_variables_skips_all_computed_block(self):
        from engine.gen_module import render_variables
...
class SampleValueObjectTest(unittest.TestCase):
    def test_list_of_object_samples_object_members_not_nested_list(self):
        ...
        from engine.gen_module import _sample_value
        self.assertEqual(
...
class UnknownNestingModeTest(unittest.TestCase):
    def test_unknown_nesting_mode_fails_loud(self):
        from engine.gen_module import _block_input_type
        with self.assertRaises(ValueError) as ctx:
...
class SampleGenerationTest(unittest.TestCase):
    """Every generated module should carry the same sample JSON that
    render_sample produces, without requiring committed demo modules."""

    def test_samples_match_generated_modules(self):
        from engine.gen_module import generate_module, render_sample
        from engine.registry import generated_types
        from engine.tfschema import load_resource
        from engine.transform import load_override
        with tempfile.TemporaryDirectory() as td:
            for rt in generated_types():
                generate_module(rt, out_root=td, fmt=False)
                generated = os.path.join(
                    td, rt, "tests", "sample.auto.tfvars.json"
                )
                self.assertTrue(os.path.exists(generated), generated)
                with open(generated, encoding="utf-8") as f:
                    got = f.read()
                want = render_sample(
                    rt, load_resource(rt), load_override(rt).get("sample")
                )
