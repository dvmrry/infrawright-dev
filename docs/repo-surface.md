...
| `packs/` | Declarative provider metadata, schemas, registries, overrides, adoption metadata, and exact distribution profiles. | Pack and distribution maintainers | Keep metadata and profiles that are validated, referenced by tests, or backed by provider-lab/readiness evidence. | Operational collector code lives in typed Node adapters. Shared pack data belongs under `packs/_shared/`; flat `*.packset.json` files validate selected pack roots without being discovered as packs. |
| `packsets/` | Byte-identical compatibility copies of the flat pack profiles for the immutable Node-v1 rollback executable. | Distribution maintainers | Do not edit independently; Go and candidate-release checks require exact parity with `packs/*.packset.json`. | Remove only when the frozen Node lane is archived. Current Go and Make defaults use the flat profiles. |
