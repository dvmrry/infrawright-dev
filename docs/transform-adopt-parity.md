...
The committed baseline intentionally exits `1`: its known differences remain
evidence gates. Unit tests pin the exact baseline and fail if a new difference
appears, an expected value changes, a classification becomes stale, or a
transform drop is introduced.
...
run where the remaining gate calls for one.
