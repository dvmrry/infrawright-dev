...
| Optional ZIA enum readback as `""` | Schema-confirmed / reported live on 4.7.26 | A private live run reported exact empty-string readback for eight optional enum fields across DLP dictionaries, HTTP header profiles, locations, and SSL inspection rules. The pinned schema confirms every affected path is optional and writable. Exact `drop_if_default: ""` rules now apply consistently in Transform, generated import config, and final Adopt projection. | Retain sanitized raw/provider-state/generated-HCL evidence for every resource on the next qualification run. Explicit `null` and nonempty values remain distinct from `""`; a provider upgrade requires requalification. |
| `zia_forwarding_control_rule.order <= 0` system-rule skip | Source-backed / reported live on `zs2` | Provider 4.7.26 requires configured order to be at least 1, while a private live run reported a protected `Fallback Mode of ZPA Forwarding` item at order `-5`. The pack retains the provider's four exact protected names and adds the numeric predicate, avoiding the observed capitalization hole without weakening the name evidence. | Retain a sanitized inventory of non-positive forwarding rules on each qualified tenant. The provider pin makes such objects unrepresentable as managed rules, but another provider version may change the valid range and must reopen this classification. |
