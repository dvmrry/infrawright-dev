...
| (conditional authoring-phase candidate) `kin-openapi` | `swagger-parser` | Optional adapter only: provider/SDK source is primary. No version or dependency is authorized until A3 verifies the exact version in Artifactory and records the review. Disable external ref/network resolution; preserve the retained OpenAPI-backed corpus; missing, partial, or unavailable OpenAPI must not block source-only evidence. |
...
and imports `go/ast` and `go/parser`, not `go/packages`. The authoring port
starts with `go/parser`, `go/ast`, and `go/token` over explicit pinned source
roots. `go/types` is added only for a concrete local-resolution gap;
`go/packages` is a later escalation requiring Artifactory validation and
review of `go list`, module loading, time/output bounds, and no-public-fallback
behavior. SSA is not part of the initial design.
...
## Authoring source-first contract gate (slice 8)

The controlling design is
[go-authoring-port-roadmap.md](go-authoring-port-roadmap.md). The missing
contract has now been decided:

```text
resource registration -> Read callback -> reachable provider calls
  -> pinned SDK source or direct raw HTTP -> method/path
  -> optional OpenAPI corroboration
```

The current AST fact collector is retained as substrate, but file association
alone is not proof: the Go analyzer roots evidence at a Read callback and
traverses only statically reachable helpers. Calls found only in Create/Delete
paths cannot support Read evidence. SDK package functions and statically proven
receiver methods are parsed through Go AST; unsupported data flow remains
unresolved rather than falling back to fuzzy success.

OpenAPI is optional. `openapi-map` remains an explicitly OpenAPI-backed view;
`reconcile` retains its API/schema/override core with optional augmentation;
`source-operation-map` and `provider-probe` work without a specification.
OpenAPI can corroborate a source-derived HTTP method/path or add labeled
suggestions, but cannot select, replace, or outrank source evidence.

The existing OpenAPI-backed Node/Python corpora remain byte authorities for
overlapping inputs. New source-only behavior has no Node equivalent and is
qualified through independently reviewed source-bound goldens; a candidate may
not generate its own expected evidence. Broad mapper and precedence parcels
receive two fresh adversarial reviews.

Six commands port. The version-specific `zpa-provider-evidence` command is
retired; its matrix and reusable pack/schema/source binding checks become a
generic analyzer corpus. This preserves the evidence without carrying a
600-line command hard-coded to one provider version.
...
| 8 | six authoring commands, source-first: absorb the AST collector; add Read-rooted provider→SDK→HTTP provenance; optional OpenAPI adapter; reconcile/evaluation/probe; transform-adopt parity; retire the ZPA-specific CLI into corpus evidence | retained Node command differentials green; independently reviewed source-only goldens; ambiguity/count and ZPA corpus gates |
| 9 | CLI shell completion: full argument/usage/exit-code surface, `fetch-diag`, remaining retained commands | full retained-command differential sweep green; retired-command routing and corpus preservation enforced |
...
| 11 | Node archive by the standard method: freeze final Node provenance, delete Node source/build/CI/release execution, retain reviewed fixtures/digests, tripwire proof | archive-plan step-4 checklist, node-substituted; no executable Node dependency |
