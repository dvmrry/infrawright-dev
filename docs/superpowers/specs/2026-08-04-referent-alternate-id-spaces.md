...
2. Cross-state output: the referent root publishes
   ...
   to `id`.
3. Edge grammar: a reference spec is a CLOSED key set — `rejectUnknownKeys`
   over `{referent, name_field}` (`metadata/packs.go:460-461`) — so the
   pack layer does not merely lack a slot for the attribute, it actively
   refuses one; declaring `referent_id_field` today is a hard validation
   failure.
4. Saved-plan output authorization: `validateOutputChanges`
   (`plan/contract.go:658`) special-cases exactly two literal output names
   (`iw_reference_ids` and its legacy spelling); every other output falls
   through to the no-op-only gate, which refuses any create.
...
- **Sidecar shape is additive.** The canonical `by_id` / `id_by_key` /
  ...
  `lookupIdentity` already canonicalizes integer identities. Unlike
  `id_by_key` — which the parser derives by inverting `key_by_id` on old
  sidecars — a missing `spaces` section is deliberately NOT derivable: the
  alternate attribute's values do not exist anywhere in the canonical id
  maps, so the only correct response to an absent space is the
  missing-lookup report (invariant 4). No fallback derivation may ever be
  added for it.
- **Output shape is a sibling, not a mutation.** The referent root
  ...
  contract is untouched — but untouched is not the same as the siblings
  being safe: the authorization gate is an allowlist on the exact output
  name, so the siblings need their own reference-output treatment
  (surface 5), and the first post-declaration plan is the case that bites.
...
5. Saved-plan authorization (site 4 above; NOT additive): the
   reference-output gate is an allowlist keyed on the exact output name,
   and a referent root's FIRST plan after declaring a space carries a
   `create` action for the new sibling output — today that plan is refused
   outright ("non-no-op output changes are not supported"). Sibling
   outputs must join the reference-output treatment: name recognition for
   `iw_reference_ids_<field>`, the create/update proven against
   provider-observed IDs through the same planned-values evidence path as
   `iw_reference_ids`, and no-op shapes accepted. This lands on the
   referent root, so without it a tenant adopting the pack change cannot
   apply. Published schema updated if it enumerates output names.
6. Pack work (rides after the engine): declare `referent_id_field: "val"`
   on the two `val`-citing edges to `zia_url_categories` —
   `zia_dlp_web_rules.url_categories.id` and
   `zia_cloud_nss_feed.url_categories.id` (the only nested-object
   referrers; the NSS feed type is not adopted on any known tenant today,
   so DLP web rules is the edge that changes behavior).
...
- A referent root's FIRST saved plan after declaring an alternate space is
  authorized and appliable: the sibling output's `create` is proven
  against provider-observed IDs, and the subsequent steady-state plan's
  no-op shape passes. Tokenization and resolution criteria alone are not
  sufficient — an implementation must not pass them while leaving the
  referent root unappliable.
