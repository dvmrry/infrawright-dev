...
- Files intentionally left untouched:
  - All JSON schemas, generated catalogs, fixtures, snapshots, and golden
    outputs.
  - Bootstrap, refresh, and acknowledgement domain publishers and all existing
    TOCTOU/CAS/crash-recovery machinery.
...
- Expected behavior change:
  ...
  - Guard removal failure becomes terminal. A primary `ProcessFailure` retains
    ...
    remains the generic `INTERNAL_ERROR` contract with the cleanup detail.
...
- Expected no-op areas: Read-only process operations, direct private domain
  helpers, Python writers, Terraform behavior, refresh pending-marker protocol,
  artifact schemas, bytes, and ordering.

## Invariants Claimed

- Evidence must not be silently dropped: No evidence path changes; guard
  failures occur before the mutation begins.
...
- Relevant output summary:
  - Final Node gate: 593 tests, 592 passed, 1 expected platform skip, 0 failed.
  ...
  - Focused final guard/bootstrap/refresh pass: 52 passed, 0 failed before the
    final full gate.
