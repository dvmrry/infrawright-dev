...
NUMERIC_INDEX = r"\[[0-9]+\]"
SELECTOR_TAIL = r"(?:\.%s|\[%s\]|%s)*" % (IDENT, HCL_STRING, NUMERIC_INDEX)
...
def _render_key_binding(key, tree, indent, items_expr="var.items"):
    base = '%s[%s]' % (items_expr, _hcl_string(key))
    return _render_merge(base, tree, indent)


def render_hcl(bindings, items_var="items",
               local_name="infrawright_expression_bound_items"):
    """Render Terraform HCL that merges expression bindings into a var map."""
    bindings = list(bindings or [])
    if not bindings:
        return ""
    if not PATH_SEGMENT.match(items_var):
        raise ValueError("items_var must be a Terraform identifier")
    if not PATH_SEGMENT.match(local_name):
        raise ValueError("local_name must be a Terraform identifier")
    ...
    sections.append("  %s = merge(var.%s, {" % (local_name, items_var))
    for key, tree in sorted(_binding_tree(bindings).items()):
        rendered = _render_key_binding(key, tree, 4, items_expr="var.%s" % items_var)
