...
// TestHTMLUnescape pins the exact behavior of
// the original implementation's pythonHtmlUnescapeGeneric,
...
// the original test corpus's "generic Python HTML
...
func TestHTMLUnescape(t *testing.T) {
	cases := []struct {
		...
		// --- ported directly from the original test corpus ---
		{
			name:  "compatibility corpus: combined named/prefix/numeric/invalid vector",
			input: "&NotEqualTilde; &notit; &#x80; &#1; &#xD800; &#xFDD0;",
			want:  "≂̸ ¬it; €  � ",
		},
		{
			name:  "compatibility corpus: single-pass double-escaped lt",
			input: "&amp;lt;",
			want:  "&lt;",
		},
		// deliberately not a table row: exercises unescaping twice, matching
		// the compatibility corpus call `pythonHtmlUnescapeGeneric(pythonHtmlUnescapeGeneric("&amp;lt;"))`.
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := HTMLUnescape(tc.input); got != tc.want {
				t.Errorf("HTMLUnescape(%q) = %q, want %q", tc.input, got, tc.want)
			}
		})
	}

	t.Run("compatibility corpus: two-pass unescape fully resolves double-escaped lt", func(t *testing.T) {
		once := HTMLUnescape("&amp;lt;")
		twice := HTMLUnescape(once)
		if twice != "<" {
