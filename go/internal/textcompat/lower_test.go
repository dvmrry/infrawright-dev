...
// `expanded` helper in the original test corpus.
...
// reviewed Node 24 Unicode tables" from the original test corpus.
...
// the Unicode compatibility contract), verifying the transcription is
...
// the original test corpus, generalized to check both the 16.0
...
// so Lower151 leaves them unchanged as a direct consequence of Go's
...
func TestPreservesRuntimeOnlyLowercaseSources(t *testing.T) {
	for _, deltaKey := range []string{"16.0", "17.0"} {
		...
		for _, cp := range additions {
			...
			character := string(cp)
			if got := Lower151(character); got != character {
				t.Errorf("%s: Lower151(%q) = %q, want unchanged (U+%04X)", deltaKey, character, got, cp)
			}
		}
		if !found {
	...
	if got, want := Lower151("꟎"), "꟎"; got != want {
		t.Errorf("Lower151(U+A7CE) = %q, want %q", got, want)
	}
	if got, want := Lower151("\U00016ea0"), "\U00016ea0"; got != want {
		t.Errorf("Lower151(U+16EA0) = %q, want %q", got, want)
	}
	if got, want := Lower151("İ"), "i̇"; got != want {
		t.Errorf("Lower151(U+0130) = %q, want %q", got, want)
	}
}

// TestFinalSigmaContext ports "Final Sigma uses Unicode 15.1 Cased and
// Case_Ignorable context" from the original test corpus verbatim.
func TestFinalSigmaContext(t *testing.T) {
	...
	for _, tc := range cases {
		if got := Lower151(tc.input); got != tc.want {
			t.Errorf("Lower151(%q) = %q, want %q", tc.input, got, tc.want)
		}
	}
	...
	if got, want := Lower151("1ʰΣ"), "1ʰσ"; got != want {
		t.Errorf("Lower151(%q) = %q, want %q", "1ʰΣ", got, want)
	}
	if got, want := Lower151("AΣʰ1A"), "aςʰ1a"; got != want {
		t.Errorf("Lower151(%q) = %q, want %q", "AΣʰ1A", got, want)
	}
}

// TestDeltaForVersion adapts "lowercase helper rejects Node Unicode runtime
// drift" from the original test corpus. Node's requireRuntimeDelta
...
// the original test corpus -- the strongest available parity
// evidence: a SHA-256 digest folding in Lower151's output for every
...
// embedded in the original test corpus (PYTHON_EXHAUSTIVE_ORACLE)
...
func TestExhaustiveDigestMatchesPythonOracle(t *testing.T) {
	...
	if got := fmt.Sprintf("%x", h.Sum(nil)); got != want {
		t.Errorf("exhaustive digest = %s, want %s (see the original test corpus)", got, want)
	}
}
