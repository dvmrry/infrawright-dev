...
// retry.go ports the original implementation: the bounded 429/Retry-After
...
// the original implementation, not by anything in this package's own
...
const (
	maxRetries = 5
	// retryCapMs ports RETRY_CAP_MS from the original implementation.
	retryCapMs = 30_000.0
)

// pythonFloat ports the PYTHON_FLOAT regexp from
// the original implementation: the same lexical grammar Python's
...
// RetryDelayMs ports retryDelayMs from the original implementation: it
