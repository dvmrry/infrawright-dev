...
// SelectAdoptionResources expands the adoption lane. It matches the transform
// lane's selection exactly: the implicit batch covers generated types plus
// active data referents, and explicit data selectors are ordinary targets.
// Data referents carry no import identity, so the adopt runner delegates them
// to the transform lane (config + lookup, never imports or moves) the same way
// it delegates derived types.
func SelectAdoptionResources(root metadata.LoadedPackRoot, selectors []string) (result TransformSelection, err error) {
	return SelectTransformResources(root, selectors)
}
