...
// scopePlanFixtureResourceSet is deliberately separate from roots_test.go's
// fixture so changed-path coverage cannot accidentally inherit its shape.
func scopePlanFixtureResourceSet() metadata.ResourceSet {
...
// scopeOnePath -- confirmed by grep over the original implementation
...
func TestChangedPathScopeNeverValidatesTenantSegments(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ChangedPathScopeFromResourceSet: %v (want success -- scope-paths does not validate tenant segments)", err)
	}
...
func TestChangedPathScopeInvalidInputErrorTexts(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, err := ChangedPathScopeFromResourceSet(ChangedPathScopeOptions{
				Paths:          c.paths,
				Workspace:      scopeWorkspace,
				DeploymentPath: scopeDeploymentPath,
				Deployment:     c.dep,
