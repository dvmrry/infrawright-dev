...
// Fetch/fetch-diag command coverage is credential-free and network-free. The
// recorded transport uses a local TLS server and fixture credentials.
...
func TestFetchRecordedTransport(t *testing.T) {
	root := repoRoot(t)
...
func TestFetchDiagValidatesHostBeforeTransportSetup(t *testing.T) {
	...
	// Invalid cloud configuration must fail before transport setup.
	testCase := struct {
		name       string
		cloud      string
		wantStderr string
