...
if (args[0] === "config" && args[1] === "get") {
  ...
  const values = {
    ...
    omit: process.env.FAKE_NPM_OMIT ?? "",
  ...
  if (packageSpec === process.env.FAKE_NPM_MISSING) {
    process.stderr.write("E404 secret-token-from-registry");
    process.exit(1);
  }
...
test("mirror manifest is derived from the pinned lockfile", () => {
  ...
  for (const target of [
    ...
    assert.match(result.stdout, new RegExp(`^  ${target}:$`, "mu"));
  }
  assert.match(
    result.stdout,
    /@esbuild\/darwin-arm64@0\.25\.12[\s\S]*@typescript\/typescript-darwin-arm64@7\.0\.2/u,
  );
  assert.match(
    result.stdout,
